...
class ComputeHostRenderTest(unittest.TestCase):
    ...
    def test_cpu_track_is_quarter_width_aggregate_with_actual_count(self):
        ...
        full = render._cpu_gauge_segs(12, 32, 8, "lvl_g")
        compressed = render._cpu_gauge_segs(12, 32, 4, "lvl_g")
        self.assertEqual((len(full[0][0]), len(full[1][0])), (3, 5))
        self.assertEqual((len(compressed[0][0]), len(compressed[1][0])), (2, 2))
        noncontiguous = render._cpu_gauge_segs(
            render._cpu_active_threads(37, 32, threads), 32, 8, "lvl_g")
        self.assertEqual(render._plain(noncontiguous), "━" + "─" * 7)
        self.assertEqual(noncontiguous[0], ("━", "lvl_g"))
