...
SHARED_UTILITY_DEFERRED="artifact-knowledge-feed.py artifact-pointer-bridge.py artifact-quiescence.py artifact-relocation-live.py artifact-relocation.py artifact_campaign.py artifact_cycle_titles.py artifact_locator_amendment.py artifact_metadata_amendment.py artifact_restore_sealed.py cairn-artifact-read.sh cairn-artifact-read.ts campaign_title_repair.py compute-hosts dispatch-readiness.py dispatch_attempt_policy.py dispatch_capacity_evidence.py dispatch_notice_receipt.py dispatch_owner_input.py dispatch_parent_completion.py dispatch_receipt_identity.py dispatch_supervision.py execution-access-diagnose.py execution_access.py execution_access_diagnose.py governor_identity.py interactive-main-recovery.py memory-post-curation-sync.py memory-store.sh opencode_session_runtime.py review_watchdog.py verification-background-lease.py work_start.py"
...
check_codex_bin_wrappers() {
  ...
  if ! grep -Fq 'run_preflight("memory"' adapters/codex/hooks/sessionstart-lifecycle.py \
    ...
    || ! grep -Fq 'from memory_session_completion import' adapters/codex/hooks/sessionend-lifecycle.py \
...
check_codex_native_hook_projection() {
  ...
  if ! grep -Fq 'material-route' adapters/codex/bin/preflight.sh \
    ...
    || ! grep -Fq 'run_preflight("material-route", "clear"' adapters/codex/hooks/sessionend-lifecycle.py; then
