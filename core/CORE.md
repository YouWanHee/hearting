...
removing, or widening an entry requires a product-spec revision. W7H (PRD v15 §31 D-85-b) moved the
content of every row into a sealed residue cycle of the campaign named in the last column; the exact
prefix now names the compat-row source and stays protected in the census and retire exclusions, and the
core-sync check (`artifact_cutover._core_sync_observation`) requires both the prefix and its relocation
locator to be declared here.

| Exact path | Meaning | Disposition class | Relocated to (W7H, 2026-09-05; campaign `campaigns/2026-09-05_2026-08-24-artifact-knowledge-index-w7/`) |
|---|---|---|---|
| `plans/2026-08-24_artifact-knowledge-index-w7/` | sealed W7 knowledge-index evidence | `C-LEG(sealed-evidence)` | `campaigns/2026-09-05_2026-08-24-artifact-knowledge-index-w7/2026-08-24_2026-08-24-artifact-knowledge-index-w7/` |
| `plans/2026-08-25_artifact-knowledge-index-w7-e1/` | sealed W7 E1 evidence | `C-LEG(sealed-evidence)` | `campaigns/2026-09-05_2026-08-24-artifact-knowledge-index-w7/2026-08-25_2026-08-25-artifact-knowledge-index-w7-e1/` |
| `plans/2026-08-25_artifact-knowledge-index-w7-e2-e3/` | sealed W7 E2/E3 evidence | `C-LEG(sealed-evidence)` | `campaigns/2026-09-05_2026-08-24-artifact-knowledge-index-w7/2026-08-25_2026-08-25-artifact-knowledge-index-w7-e2-e3/` |
| `plans/2026-08-25_artifact-write-cutover-w7c/` | sealed W7C cutover evidence | `C-LEG(sealed-evidence)` | `campaigns/2026-09-05_2026-08-24-artifact-knowledge-index-w7/2026-08-25_2026-08-25-artifact-write-cutover-w7c/` |
| `spec/artifact-path-contract/_internal/research/` | retained artifact-path-contract research evidence | `C-LEG(sealed-evidence)` | `campaigns/2026-09-05_2026-08-24-artifact-knowledge-index-w7/2026-08-24_artifact-path-contract/` |
| `research/hermes-agent/.gitignore` | retained research repository-control evidence | `C-LEG(sealed-evidence)` | `campaigns/2026-09-05_2026-08-24-artifact-knowledge-index-w7/2026-06-29_hermes-agent/` |

**Write cutover.** The legacy `C-DUR` buckets above (`analysis_project/`, `research/`, `spec/`, `plans/`, `documents/`, `experiments/`, `designs/`) are writable only while the producer cutover is inactive (`.runtime/artifact-producer/v1/cutover.json` absent). Once activated by the approval package, every new write must land under an open cycle's `campaigns/<campaign-locator>/<cycle-locator>/artifacts/`, IDs are issued by `begin` before the first write, and `artifact_producer.py check-write` is the single allow/deny oracle for hooks and writers. The approval-gated follow-ups run through `utilities/artifact_cutover.py`: `migrate-delta`/`migrate-seal` copy the census-classified legacy delta into one sealed cycle and new shared revisions (sources preserved, journaled), `compat-close` records the map set legacy readers resolve through (`resolve-legacy`; the spec gate falls back to the latest `shared/spec/` revision), and `retire` deletes only digest-verified, backed-up sources. Existing legacy content stays readable; nothing is moved or deleted by activation. Readers resolve a bucket through `utilities/artifact_reader.py` (cycle dirs → latest shared revision → the legacy bucket as a read-only fallback, missing legacy paths via `resolve-legacy`); a reader that still opens `<root>/<bucket>/` directly sees only the retirement exclusions. A gated lump cycle from an earlier migration is retro-split into D-23-bounded cycles through `artifact_resplit.py resplit-legacy-cycle` (gates `r1`/`r2`/`r3`), never by editing the lump tree in place. `fleet_cutover_gate.py complete --require-resplit` additionally blocks a root that still has an unresplit lump remaining. The historical `campaigns/<camp-id>/cycles/<cyc-id>/` layout is moved into readable `<date>_<slug>` locators by `utilities/artifact_relayout.py` (`plan` is a pure read; `apply` is a journaled same-filesystem rename with per-operation inverse rows, D-92 retroactive naming, and a D-82 compat-map append so old ID locators keep resolving; `resume`/`rollback` replay the journal; a nonterminal run is a typed `relayout-in-progress` hold that `artifact_reader.py hold` reports). Completion closes the D-91 transition window for that root, after which `begin` refuses a slugless route (`route-slug-missing`), and `fleet_cutover_gate.py complete --require-relayout` blocks a root that still has a legacy-shaped campaign or cycle directory. What W7C/W7G left at the legacy top level (stage internals, project materials, route containers, sealed W7 evidence, trash) is disposed of by `utilities/artifact_residue.py` (`plan` pure read; `apply --route-file` derives one route per residue cycle, moves files by rename into sealed residue cycles that cite their origin, into `documents` cycles, or into `.runtime/routes/legacy/`, appends compat rows and re-points a moved map file; `retire-trash --approval` is the only deletion path and backs up first; a live symlink whose siblings already migrated rejoins them by rename — link bytes preserved, a `kind: symlink` compat row with `relocated_from`, the sealed manifest untouched — while a link with no mapped ancestor, a relative link that would not resolve at its new home, and a dangling link stay typed deferrals; `spec/` stays a typed deferral while consumers still read it, and moves as root support residue only under `--include-spec-top`); `fleet_cutover_gate.py complete --require-residue` blocks a root whose top level holds anything but typed deferrals.
