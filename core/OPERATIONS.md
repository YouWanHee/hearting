...
SD items retain actionable rules; full decision records are in `core/ADAPTATION.md §8`.
...
**Main/worker bootstrap boundary.** Every registered headless dispatch and repo-owned background model caller exports `AGENT_SESSION_ROLE=worker` before launch; legacy adapter markers remain fail-closed worker evidence. Input is `roles/worker-bootstrap.md`, exactly one `roles/worker-types/{owner,stage,review,support}.md`, and the assigned capability/stage contract. `worker_type` selects the fragment; capability plus assigned contract/route node select work; `model_role` selects behavior; sealed `model_profile` selects budget. These and depth are separate axes, never bootstrap aliases; `worker_role` is legacy read-only metadata. All depth-1/depth-2, loop/drill, title/distill and native-subagent workers retain safety, permission, scope, route, handoff, liveness and verification guards. They never manufacture main sessions or launch shutdown curators. The harness adds no main response/confirmation, automatic memory/briefing/nudge/distill, SessionEnd sync/curation, Fleet title/token/UI context, full capability catalog, unrelated stages, integration/merge/push/cleanup or user explanation.

Lifecycle suppression, controlled prompt isolation and physical masking are separate claims. Remove controlled full-main-bootstrap reads. When runtime-inherited project instructions lack a verified worker-disable switch, document residual input and use the minimal typed overlay; never claim full masking. Details belong in canonical artifacts; final output is exactly three lines: `artifact: <path|->`, `verdict: PASS|FAIL|BLOCKED`, `blocker: none|<one line>`. Material registered work requires an artifact; `-` is atomic read-only support only.
...
the proposed loss: before deleting each unproven candidate, the updater must
append its non-recoverable gap to the canonical `inventory/gaps.jsonl` with
...
2. **Execution and naming:** create the worktree with `git worktree add <path> -b <slug> origin/<base>` using §5.9 base selection. The sole canonical path is the sibling directory `<repo>-wt/<slug>`, such as `Foo-wt/<slug>` for `Foo`; do not invent `<repo>_worktrees/`. `worktree-path-guard` hard-enforces this naming for `git worktree add`, while `WORKTREE_GUARD_BYPASS=1`, non-add subcommands, and non-git contexts fail open.
   ...
   - **Owner binding 거부 안내:** `owner-route-binding-tuple-invalid`는 현재/기대 dispatch depth, worker type, raw route-file 유무와 위반 조건 각각을 출력한다. owner 환경에서 depth-2 자식을 직접 wrapper에 넘긴 호출자는 `stage-dispatch-fallback.py --route <route> --node <node> --slug <slug> --parent <owner> --start`를 사용한다. 봉인된 병렬 그룹은 `dispatch-batch.py --route <route> --parallel-group <group> --slug-prefix <prefix> --parent <owner> --action start`를 사용한다. 이 호출자가 raw `--route-file`을 직접 추가하거나 owner 환경을 수동 제거하도록 안내하지 않는다. 정식 launcher 내부의 node binding 전달은 유지한다.
   ...
   - **Execution access is separate from permission posture:** an optional `execution_access_v1` file declares exact writable roots, read roots, and network need for one launch. The wrapper accepts it only through `--execution-access-file` or `AGENT_DISPATCH_EXECUTION_ACCESS_FILE`, validates and hashes it before registration or model spawn, and refuses malformed, broad, symlink-escaping, parent-expanding, or unsupported requests instead of retrying without the request. No request preserves existing grants and argv byte-for-byte. Runtime projection is honest rather than flag-shaped: Codex uses `--add-dir` for `codex exec` and `--writable-root` for App Server supervision; current Claude and OpenCode path controls are `tool-permission`, not OS sandboxes, and their network grade is `none`. A boolean Codex network grant is not a host allowlist. Requests never merge with approval keys, enable blanket bypass/full-access, edit user config, or widen a child's inherited parent boundary. Until the propagation slice lands, this contract applies to the single wrapper launch that directly received the request and does not claim owner-to-worker or continuation delivery.
   - **Registered headless permission posture:** registered Claude owners/workers run `claude -p`, inheriting `permissions.defaultMode` from managed/project-local/project/user settings, defaulting to Manual only when unconfigured. Interactive plan-mode `auto` defaults do not apply to `-p`; Hearting's template instead seeds user settings with `auto`. Repeated classifier blocks kill unattended headless work without a typed cause (2026-09-02 att-e836ebd264a7400a803577a392dcc222: `3 consecutive actions were blocked`, then `ls`, `echo`, `capability-route complete`, `--help` refused). The wrapper therefore pins start posture. Shipped `headless.claude_permission_mode: bypass` in `profiles/dispatch-defaults.yaml` (user `~/.config/hearting/dispatch-defaults.yaml`, per-launch `--permission-mode` / `CLAUDE_DISPATCH_PERMISSION_MODE`) adds `--permission-mode bypassPermissions` to registered `-p` and supervisor first/resumed turns. `allowlist` opts out and is automatic fallback for root/sudo or `permissions.disableBypassPermissionsMode: "disable"` in any honored scope: managed, `<worktree>/.claude/settings.local.json`, `<worktree>/.claude/settings.json`, user. It pins `--permission-mode acceptEdits` for file/common filesystem actions in worktree/added artifacts, plus exact `--allowedTools`: sealed-AGENT_HOME `capability-route`, `dispatch-node`, `dispatch-batch`, `dispatch-owner`, `dispatch-harvest`, `stage-dispatch-fallback`, `artifact_producer`, `spec-transaction`; read-only git; owner-only commit-expected `git add`/`git commit`; harness test runners; `Edit(//<worktree>/**)` / `Edit(//<artifact-root>/**)` (also Write). These allow rules accompany bypass and take effect on runtime demotion. Record applied `permission_mode=`, `permission_mode_reason=`, `permission_inherited_mode=` in row/receipt, not just intent. Deny rules, including fatal async `--disallowedTools` (SD-71/78), remain in every mode. Registered Codex keeps sandbox security with `approval_policy=never`; never use `--dangerously-bypass-approvals-and-sandbox` there. Managed **interactive** `codex`/`resume`/`fork` via `utilities/codex-launcher.py` instead defaults to bypass (counterpart of `steward.child_permission_mode`), with `AGENT_CODEX_INTERACTIVE_PERMISSION_MODE=inherit` or caller posture opting out (`adapters/codex/ADAPTATION.md`). OpenCode's scoped `permission.external_directory` allow/deny avoids headless auto-reject through deny, not bypass.
   - **Checked evidence probe — whose parent and worktree are being probed:** create the final isolated worktree first and probe that exact canonical path. Each `nested-dispatch-eligibility.py` child tuple seals `checked_worktree`; compile requires route-cwd equality. Primary/staging/replaced worktrees supply no reusable proof. Parent fields describe the prospective depth-1 registered-headless **launcher of the depth-2 child**, not the depth-0 probing session. `--parent-transport auto` resolves headless (explicit interactive contradicts it); `--parent-sandbox auto` resolves that wrapper's canonical `AGENT_DISPATCH_CURRENT_SANDBOX`; `--parent-harness` is required, with auto only inside an identity-exporting wrapper. All resolve or fail closed. Active Codex owners require actual `AGENT_NESTED_HEADLESS_NETWORK=1`. Before owner launch, depth 0 must use `utilities/dispatch-readiness.py`: it automatically applies `--prospective-standard-owner --jobs <canonical-jobs.log>`, proves exact registry/lock writable, applies the launcher's standard+ owner predicate, labels prospective proof and atomically emits the complete object without fabricating a runtime marker or hand-assembling tuples. A raw active-owner probe lacking that marker is `prospective-owner-check-required`, not global network unavailability or proof an explicit Codex owner is unsupported. Launch cross-checks parent fields, worktree, canonical registry and final owner sandbox before paying for an owner. Pass `dispatch-owner --route-evidence <route.json>` so the selector-only cascade stays within probed owner harnesses; this flag does not cross the wrapper boundary. Sealing depth-0 values instead blocks hops 1/2 for every node/adapter with `dispatch-evidence-parent-runtime-mismatch` or `parent-attempt-not-found`, without authorizing inline work (2026-07-31 sandbox, 2026-08-04 transport incidents).
   ...
   - **Review verdict is a result, not a worker death (SD-94 owner-closure extension):** a readable exact review FAIL is a completed blocking round, never a dead worker. The shared `complete` writer admits an exhausted-round owner disposition only after verifying review identity, terminated budget, every blocking finding's linked evidence, and the destination cycle. For an official continuation, the same operation verifies immutable source-route/hash and node-contract lineage, counts inherited rounds without resetting their budget, and publishes an explicitly owner-overridden gate only in the continuation. Its current-cycle record cites the source reviews; source review rows, FAIL results, markers and sealed payloads remain unchanged. `complete --check` evaluates owner-closure admission without publication or state writes; normal completion rechecks before committing. A refusal reports the exact unsatisfied obligation. See `docs/continuation-review-closure.md` for the evidence and recovery contract.
...
`to{harness,session_id|name}`, `kind`, `summary` (first line, hard-truncated to 200
chars), `body_sha256` (complete-body sha256), `delivery{surface,status,receipt}`,
`refs[]`. S6's body-storage prohibition applies to every field.
...
| Codex | `herdr agent wait <target>` — measured 2026-09-02 (cairn-codex-test, idle matches immediately); `--until working/idle --timeout` timed out after completion in the 2026-09-03 F-100 test — read the pane instead | detached `watch` + next-turn receipt pull; no wake carrier — unknown (no capability claim before measurement, P-7) | `peer-steward.py prompt` (herdr, F-100c) — native path retired (`codex queue` / gateway `steer` op not implemented) | herdr prompt with the `(peer-from: …)` trailer → `userprompt-lifecycle.py` writes the `notice` | `peer-steward.py` record + Codex hook `notice` | send/receive measured 2026-09-03 / steward wake unknown |
| OpenCode | `herdr agent wait <target>` — unknown (pending P-6) | unknown | `peer-steward.py prompt` (herdr, F-100c) | herdr prompt with the `(peer-from: …)` trailer → `hearting-guards.js` `chat.message` writes the `notice` | `peer-steward.py` record + plugin `notice` | send measured 2026-09-03 / receive pending live measurement / steward unknown |
