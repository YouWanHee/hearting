...
**W7C cycle layout.** The output paths above are bucket-relative. While the producer cutover is inactive they resolve to the legacy top-level bucket; once active they resolve to `campaigns/<campaign-locator>/<cycle-locator>/artifacts/<bucket>/…` of the cycle issued by `utilities/artifact_producer.py begin`; the machine-managed `.cycle.json` beside `artifacts/` binds that renameable locator to its stable IDs and records its start time (see `core/CORE.md §3` and `producer_lifecycle` in `capabilities/topologies.json`). Only `spec`, `analysis`, and explicitly promoted `research` reach the immutable `shared/` tree, via `admit-shared` from a sealed cycle.

**Campaign or cycle.** A cycle is the deterministic D-23 unit of one route's work; a campaign is the agent's proposal for the work stream. `compose` demands that proposal: pass `--campaign-key <stream>` (`artifact_producer.py campaign-list` and the `[경로]` notice show the root's active keys, and `compose` refuses with `compose-campaign-key-required` when none is named), `--parent-cycle` to inherit the predecessor's campaign, or `--unassigned` to opt out explicitly; the same `compile` option remains optional. The sealed route carries the choice to the owner's `begin`. Explicit `begin --campaign-key`/`--campaign` remains supported and must agree with the route and parent. With no selection, `begin` uses the root's `_unassigned` campaign with `degraded=true`; this container asserts no shared goal. A new campaign's locator, slug and title come from its key (`<date>_<key-slug>`, `slug_source=campaign-key`); the route slug names only the cycle under it. `--parent-cycle` may name an open or sealed cycle of the same active campaign and records its state at admission. It carries causality only, never permission to consume mutable inputs or skip completion gates. A linked child may produce output while its parent is open, but final manifest admission waits for the parent to seal; use the shared key without a parent edge for independent work. Name the stream, such as `tts-v6-release`, in the key and the bounded task in the date-free slug; record the predecessor and exact input revision to preserve why the next cycle exists. Session, capability and route-family identity never imply one campaign. Existing campaigns are unchanged; artifact-path-contract §27.2 owns the complete contract. A cycle's work closes in the order `complete` → `close` → `finalize`; a cycle sealed before its route closed cannot become `completed`, and campaign closure discloses it as `sealed-unproven`.

**Campaign size.** A campaign is a stream whose closing condition can be stated in one sentence when `compose` names the key — `v4.1 revision: closed when co-author feedback is applied`, `tts-v6-release: closed when v6 ships`. A key that cannot close within weeks is too large (a project or repository name is not a key); work that ends with one cycle is too small (join the stream it serves). Follow-up cycles name their predecessor with `--parent-cycle` so a round reads as one chain, and `artifact_producer.py campaign-close` closes the stream once the condition is met. Dozens of active keys signal unclosed or over-split streams, not an invitation to join the largest one.
