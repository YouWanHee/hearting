...
resolve_source_root() {
  if [ -n "${AGENT_HOME:-}" ]; then if accepted=$(is_harness_source_root "$AGENT_HOME" 2>/dev/null); then printf '%s\n' "$accepted"; return 0; fi; fi
...
PY
  ...
  for candidate in "$git_candidate" "$relative_candidate"; do [ -n "$candidate" ] || continue; if accepted=$(is_harness_source_root "$candidate" 2>/dev/null); then printf '%s\n' "$accepted"; return 0; fi; done
  return 1
}
