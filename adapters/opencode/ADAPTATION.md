...
| local evidence exposure | The per-turn context blocks also run `preflight.sh local-evidence [cwd]`, the portable `hooks/local-evidence-inject.sh` presence probe: research/documents/analysis bucket counts plus at most six newest entry paths from the canonical artifact root (2,400-UTF-8-byte bound, no body reads, no prompt classifier, silent when empty, worker-exempt, fail-open) |
