...
| `top` | configured top tier / `xhigh` | exception above deep: the main-session-only model (`CFG_MAIN_SESSION_ONLY_MODELS`, enforced by this wrapper since 2026-09-10), reachable only from a route that seals the explicit `top` profile on a dispatch-depth-1 owner or review worker; no cascade in or out |
