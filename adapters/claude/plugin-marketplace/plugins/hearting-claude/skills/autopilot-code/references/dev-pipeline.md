...
On a gate-bearing route — a `standard+` route whose compiled graph retains the
`frame-review` binding, which is an explicit `hybrid`/`both`/`post-frame-only`
`confirmation.mode` or any composed recipe that declares the gate — the
post-frame direction gate follows (owner-execution.md "Post-Frame Direction
...
does not start before that — every launch surface refuses it. Under
`autonomous` a non-composed `standard+` route seals no `frame-review` binding,
`frame` continues `inline-next`, and `plan` starts immediately after the frame
group joins; `direct`/`quick` have no `frame` node at all.
...
STAGE_PROMPT="<sub-skill contract + absolute input paths (Intent: shards/frame/intent.md first when the route raised the frame-review gate, then every direction brief) + output contract + slug>"
