...
def main(argv: list[str]) -> int:
    ...
    # loosen it, so consuming it before the tuple-validity check at :1392 is
    ...
    if action == "start" and args.replica_batch_expectation is not None:
        try:
            ...
            return fail(exc.reason, 65, detail=exc.detail, child_spawned="0")
    prompt_name = (
        f"{args.slug}.{args.command_attempt_id}.claude.prompt.txt"
        if args.command_attempt_id
        else f"{args.slug}.claude.prompt.txt"
    )
    prompt_path = log_dir / prompt_name
    log_name = (
        f"{args.slug}.{args.command_attempt_id}.claude.jsonl"
        if args.command_attempt_id
    ...
    if action == "start" and args.attempt_claimed:
        ...
        if args.launch_lifecycle == FOREGROUND_SCOPED:
            ...
            if death:
                ...
                if reason != "capacity":
                    write_reset_cache(agent_home, "claude", reason, reset, jobs)
