...
def _apply_notice(prompt: str, notice: str) -> str:
    ...
    JSON. Used at the two continuation sites whose prompt builders
    (`start_retry_prompt`, `remediation_prompt`) carry no receipt at all, so
...
def main(argv: list[str] | None = None) -> int:
    ...
    # SD-116 (c): a one-element list, not a bool, so `_seal_terminal_handoff_or_raise`
    ...
    try:
        ...
        while True:
            ...
            if new_attempts:
                ...
                # that let `attempt_stage_advance` negotiate `3` internally is
                # the negotiation the model-facing receipt must honor too --
                # `receipt_with_stage_advance` is a no-op unless both
                # `negotiated` is true AND an outcome=="advanced" record was
                # actually found, so an un-negotiated or refused round stays
                # byte-identical v2.
                receipt = receipt_with_stage_advance(
                    receipt,
            ...
            if unresolved:
                ...
                next_prompt = _apply_notice(remediation_prompt(unresolved, jobs=args.jobs), notice)
                continuations += 1
                resume = True
                continue
