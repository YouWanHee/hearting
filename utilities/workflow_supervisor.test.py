...
class TestGateSubjectNotCaller(WorkflowFixture):
    ...
    def test_release_retires_a_record_the_hook_still_holds(self):
        ...
        for carrier_state in ("claimed", "sent-ambiguous"):
            with self.subTest(carrier_state=carrier_state):
                route, path = self.two_stage_route(human_gate="frame-review")
                jobs, session, _attempt = self.owner_registry(route_id="rt-fixture0000000")
                root = Path(jobs).resolve(strict=False).parent
                out = io.StringIO()
                ...
                self.assertEqual(PENDING.read(root, session, delivery_id)["state"],
                                 carrier_state)
                self.assertEqual(
