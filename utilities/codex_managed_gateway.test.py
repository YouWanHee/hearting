...
import codex_managed_dispatch as MANAGED
...
class ManagedGatewayTest(unittest.TestCase):
    ...
    def test_resume_with_self_parent_does_not_bind(self) -> None:
        self.server.thread_resume_forked_from_id = "thread-resumed"
        self.client.request("thread/resume", {"threadId": "thread-resumed"})
        status = control(self.control, {"schema_version": 1, "op": "status"})
        self.assertEqual(status["thread_id"], "thread-1")
        self.assertEqual(status["binding_generation"], 1)
    ...
    def _unprove_latest_transition(self) -> None:
        self.server.next_start_id = "thread-unproved"
        self.server.thread_start_session_id = "thread-contradiction"
        self.client.request("thread/start", {})

    def test_unproved_transition_blocks_old_binding_until_verified_resume(self) -> None:
        notice = self._human_gate_request()
        completion = receipt_request("batch-before-unproved")
        before = self.server.counts()
        self._unprove_latest_transition()
        status = control(self.control, {"schema_version": 1, "op": "status"})
        self.assertEqual(status["thread_id"], "thread-1")
        self.assertTrue(status.get("binding_blocked"))
        with self.assertRaises(MANAGED.ManagedDispatchError) as raised:
            self._probe_from_launcher()
        self.assertEqual(raised.exception.reason_class, "transition-unproved")
        for request in (completion, notice):
            result = control(self.control, request)
            self.assertEqual(result["status"], "rejected", result)
            self.assertEqual(result["reason"], "transition-unproved", result)
        self.assertEqual(self.server.counts(), before)
        self.client.request("thread/resume", {"threadId": "thread-1"})
        self.assertEqual(self._probe_from_launcher().thread_id, "thread-1")
        self.assertEqual(control(self.control, completion)["status"], "accepted")

    def test_prepared_completion_and_notice_recheck_unproved_binding(self) -> None:
        requests = [receipt_request("batch-prepared-unproved"), self._human_gate_request()]
        pending = []
        for request, validation in zip(requests, (self.gateway._validate_delivery, self.gateway._human_gate_validation)):
            thread_id, parent, batch, receipt, digest, delivery = validation(request)
            identity = dict(thread_id=thread_id, parent_attempt_id=parent,
                            sealed_batch_id=batch, receipt_digest=digest)
            item = GATEWAY.PendingInternal(
                kind="completion-wait" if not pending else "human-gate-wait",
                thread_id=thread_id, delivery_id=delivery, identity=identity,
                receipt=receipt, recipient_epoch=self.gateway._epoch,
                binding_generation=self.gateway._binding_generation)
            with self.gateway._lock:
                self.gateway.ledger._transition(delivery, "prepared", **identity)
                self.gateway._delivery_pending[delivery] = item
            pending.append(item)
        before = self.server.counts()
        self._unprove_latest_transition()
        with self.gateway._lock:
            state = self.gateway._threads["thread-1"]
            state.active_turn_id = "turn-existing"
            state.queued.extend(pending)
            self.gateway._drain_queued_locked(state)
        for item in pending:
            self.assertTrue(item.event.wait(5))
            self.assertEqual(item.outcome["reason"], "transition-unproved")
        self.assertEqual(self.server.counts(), before)

    def test_failed_rpc_and_stale_response_do_not_poison_binding(self) -> None:
        self.server.thread_start_error = True
        with self.assertRaises(AssertionError):
            self.client.request("thread/start", {})
        self.assertEqual(self._probe_from_launcher().thread_id, "thread-1")
        with self.gateway._lock:
            self.gateway._track_transition_response_locked(
                ("int", 900), "thread/start", {},
                {"result": {"thread": {"id": "thread-stale", "sessionId": "thread-contradiction"}}},
                {"epoch": self.gateway._epoch, "sequence": self.gateway._latest_transition_sequence - 1,
                 "prior_thread_id": "thread-1"})
        self.assertEqual(self._probe_from_launcher().thread_id, "thread-1")
        self.assertEqual(control(self.control, receipt_request())["status"], "accepted")

    def test_resume_accepts_persistent_fork_metadata_without_promoting_it(self) -> None:
        self.server.thread_resume_forked_from_id = "thread-original-parent"
        self.server.thread_resume_session_id = "thread-original-root"
        self.client.request("thread/resume", {"threadId": "thread-resumed"})
        status = control(self.control, {"schema_version": 1, "op": "status"})
        self.assertEqual(status["thread_id"], "thread-resumed")
        self.assertEqual(status["binding_generation"], 2)
        self.assertEqual(status["binding_session_id"], "thread-original-root")
        self.assertNotIn("thread-original-parent", status["thread_ancestors"])
        self.assertNotIn("thread-original-root", status["thread_ancestors"])

    def test_resume_preserves_witnessed_fork_metadata(self) -> None:
        self.client.request("thread/fork", {"threadId": "thread-1"})
        self.server.thread_resume_forked_from_id = "thread-1"
        self.client.request("thread/resume", {"threadId": "thread-2"})
        status = control(self.control, {"schema_version": 1, "op": "status"})
        self.assertEqual(status["thread_id"], "thread-2")
        self.assertEqual(status["binding_generation"], 2)
        self.assertEqual(status["last_transition"]["status"], "committed")

    def test_resume_rejects_contradiction_of_witnessed_fork(self) -> None:
        self.client.request("thread/fork", {"threadId": "thread-1"})
    ...
    def test_same_thread_resume_preserves_launched_batch_generation(self) -> None:
        pending_batch = receipt_request("batch-before-resume")
        self.client.request("thread/resume", {"threadId": "thread-1"})
        status = control(self.control, {"schema_version": 1, "op": "status"})
        self.assertEqual(status["binding_generation"], pending_batch["binding_generation"])
        delivered = control(self.control, pending_batch)
        self.assertEqual(delivered["status"], "accepted")
        self.assertEqual(self.server.start_count, 1)

    def _probe_from_launcher(self):
        return MANAGED.probe_managed_codex_parent(
            parent_harness="codex", parent_session_id="thread-1",
            environ={"AGENT_CODEX_MANAGED_GATEWAY": "1",
                     "AGENT_CODEX_MANAGED_PARENT_RUNTIME": "codex",
                     "AGENT_CODEX_MANAGED_CONTROL_SOCKET": str(self.control),
                     "CODEX_THREAD_ID": "thread-1"})

    def test_revisited_conversations_preserve_launcher_authority(self) -> None:
        for target in ("thread-b", "thread-c", "thread-b", "thread-1"):
            with self.subTest(target=target):
                self.client.request("thread/resume", {"threadId": target})
                status = control(self.control, {"schema_version": 1, "op": "status"})
                ancestors = status["thread_ancestors"]
                self.assertEqual(status["thread_id"], target)
                self.assertEqual(len(ancestors), len(set(ancestors)))
                self.assertNotIn(target, ancestors)
                self.assertEqual(self._probe_from_launcher().thread_id, target)

    def test_bounded_transition_history_keeps_launcher_authority(self) -> None:
        for ordinal in range(GATEWAY.MAX_THREAD_LINEAGE + 4):
            target = f"thread-history-{ordinal}"
            self.client.request("thread/resume", {"threadId": target})
        status = control(self.control, {"schema_version": 1, "op": "status"})
        ancestors = status["thread_ancestors"]
        self.assertLessEqual(len(ancestors), GATEWAY.MAX_THREAD_LINEAGE)
        self.assertEqual(len(ancestors), len(set(ancestors)))
        self.assertIn("thread-1", ancestors)
        self.assertEqual(self._probe_from_launcher().thread_id, target)
        ...
        if final["thread_id"] == "thread-sibling":
            ...
            if sibling_outcome["status"] == "rejected":
                self.assertIn(sibling_outcome["reason"], {
                    "stale-binding-generation", "thread-not-owned-by-current-tui", "transition-unproved",
                })
        else:
...
class EstablishFromEmptyBindingRaceTest(unittest.TestCase):
    ...
    def test_initial_explicit_fork_establishes_binding(self) -> None:
        self.server.thread_fork_session_id = "thread-tree-root"
        self.client.request("thread/fork", {"threadId": "thread-launch-source"})
        status = control(self.control, {"schema_version": 1, "op": "status"})
        self.assertEqual(status["thread_id"], "thread-2")
        self.assertEqual(status["binding_generation"], 1)
        self.assertEqual(status["thread_ancestors"], ["thread-launch-source"])
        self.assertEqual(status["binding_session_id"], "thread-tree-root")
        self.assertNotIn("thread-tree-root", status["thread_ancestors"])

    def test_initial_fork_rejects_mismatched_response_parent(self) -> None:
        self.server.thread_fork_parent_id = "thread-forged-parent"
        self.client.request("thread/fork", {"threadId": "thread-launch-source"})
        status = control(self.control, {"schema_version": 1, "op": "status"})
        self.assertEqual(status["thread_id"], "")
        self.assertEqual(status["binding_generation"], 0)
        self.assertEqual(status["thread_ancestors"], [])
        self.assertEqual(status["last_transition_reason"], "forked-from-contradiction")
