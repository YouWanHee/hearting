...
if "$CODEX" headless >"$TMP/logs/codex_headless.out" 2>"$TMP/logs/codex_headless.err" \
  ...
  && grep -q '^model_selection_surface=--model-profile <deep|balanced-deep|balanced|light|mini> \[--model-role <portable-role>\]|--model-role <portable-role>|--model <model> --reasoning <effort>$' "$TMP/logs/codex_headless.out" \
...
if "$CODEX" dispatch --dry-run --worktree "$TMP/repo" --slug codex-inherit-model --capability autopilot-code --mode dev --qa standard --prompt-text "do work" --inherit-model-settings --jobs "$TMP/codex-inherit-model.log" >"$TMP/logs/codex_inherit_model.out" 2>"$TMP/logs/codex_inherit_model.err"; then
  bad "codex dispatch wrapper should reject unprovable model inheritance"
else
  rc=$?
  if [ "$rc" -eq 64 ] \
    && grep -q '^reason=headless-model-inheritance-ineligible$' "$TMP/logs/codex_inherit_model.out" \
...
if AGENT_HOME="$ROOT" "$OPENCODE" material-route check --tool Write --file "$opencode_source" --cwd "$TMP/repo" --session opencode-no-route >"$TMP/logs/opencode_mr.out" 2>"$TMP/logs/opencode_mr.err"; then
...
if OPENCODE_SESSION_ID=opencode-bind AGENT_HOME="$ROOT" AGENT_DISPATCH_JOBS="$MATERIAL_ROUTE_JOBS" \
  ...
  && [ -f "$opencode_route" ] \
  && AGENT_HOME="$ROOT" "$OPENCODE" material-route check --tool Write --file "$opencode_source" --cwd "$TMP/repo" --session opencode-bind >"$TMP/logs/opencode_bind_allow.out" 2>"$TMP/logs/opencode_bind_allow.err"; then
...
if AGENT_HOME="$ROOT" "$OPENCODE" material-route check --tool Write --file "$opencode_source" --cwd "$TMP/repo" --session opencode-foreign >"$TMP/logs/opencode_foreign.out" 2>"$TMP/logs/opencode_foreign.err"; then
